...
export function Step4Summary() {
    const { state, summary } = useQuoteBuilder();

    // Local state for AI summary since it's not in QuoteBuilderState
    ...
    const generateSummary = async () => {
        ...
        try {
            const response = await aiService.generateExecutiveSummary({
                ...
                services: state.items.map(i => ({
                    name: i.serviceName,
                    price: i.clientPrice
                })),
                totalPrice: summary.totalClientPrice,
    ...
    useEffect(() => {
        if (!executiveSummary && state.items.length > 0) {
            generateSummary();
        }
    }, []);
                ...
                <div className="lg:col-span-1 space-y-6">
                    <Card className="p-6 glass-card space-y-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                <p className="text-sm text-gray-500">Precio Total (Cliente)</p>
                                <AnimatedCounter
                                    value={summary.totalClientPrice}
                                    currency={state.currency}
                            ...
                            <div className="pt-4 border-t border-gray-100">
                                <MarginIndicator margin={summary.netMarginPercent} />
                            </div>
                        </div>
                    </Card>

                    <Card className="p-6 glass-card">
                        <h3 className="font-bold text-gray-900 mb-4">Desglose de Servicios</h3>
                        <ul className="space-y-2 text-sm">
                            {state.items.map(item => (
                                <li key={item.id} className="flex justify-between items-center bg-white/40 p-2 rounded">
                                    <span className="truncate max-w-[150px]">{item.serviceName}</span>
                                    <span className="font-medium text-gray-600">
                                        {item.pricingType === 'hourly' ? `${item.estimatedHours}h` : `x${item.quantity}`}
                                    </span>
                                </li>
                            ))}
                        </ul>
                ...
                <div className="lg:col-span-2">
                    <Card className="p-6 glass-card h-full flex flex-col">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            >
                                {isGenerating ? <Loader2 className="animate-spin w-4 h-4" /> : 'Regenerar'}
                            </Button>
                        </div>
