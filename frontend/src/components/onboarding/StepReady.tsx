...
import { completeOnboarding, hasOnboardingAuth } from '@/services/onboardingApi';
...
export function StepReady({ data, onGoToDashboard, onCreateQuote }: StepReadyProps) {
    ...
    const monthlyFixedCosts = data.fixedCosts?.totalMonthly ?? 0;
    const salaryWithCharges = data.team?.salaryWithCharges ?? (data.team?.applySocialCharges ? (data.team?.salary ?? 0) * 1.52852 : (data.team?.salary ?? 0));
    const monthlyPayroll = salaryWithCharges;

    const [billableHoursPerWeek, setBillableHoursPerWeek] = useState<number>(data.team?.billableHours ?? 28);
    const vacationDays = data.team?.vacationDays ?? 20;
        ...
        <div className="space-y-8 max-w-3xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 justify-center">
                ...
                <Button
                    ...
                    disabled={isSaving}
                >
                    {isSaving ? 'Guardando...' : 'Crear mi Primera Cotización →'}
                </Button>
            </div>
        </div>
    );
